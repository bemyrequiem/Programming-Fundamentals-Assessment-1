...
To clear the tile type 'clear'
 ...
 Oops... You failed to solve puzzle %s.
    ...
    def getLeastSolutions(self, solutions):
        m = min(solutions, key=len)
        print(m)
        print(m % 9, m - (m % 9))
        return m % 9, m - (m % 9)
    ...
    def checkSolutions(self):
        ...
        for y in range(0, 9):
            for x in range(0, 9):
                if self.grid[y][x] == " ":
                    solutions.append(self.findSolutions(x, y))
                else:
                    solutions.append(["#", "#", "#", "#", "#", "#", "#", "#", "#", "#"])
        print(solutions)
        return solutions
    ...
    def computerSolve(self): # backtracking algorithm to solve the grid from it's current point
        ords = self.getLeastSolutions(self.checkSolutions())
        print(ords)
        x = ords[0]
        y = ords[1]
        print(x, y)
            ...
            while 1:
                ...
                try:
                    ...
                    if chosen_option == "1":
                        ...
                        print("\nCompleted in {:.4f} seconds".format(finish - start))
                    else:
                        start = time.perf_counter()
                        try:
                            game.computerSolve()
